...
export class CompanyAuthUseCases implements ICompanyAuthUseCases {
  constructor(
  ...
  async preRegistration(userData: PreRegistrationDTO): Promise<void> {
    const { email, username, password, phone, referredReferralCode } = userData;
  ...
  async verifyOtpAndRegister(email: string, otp: string): Promise<void> {
    ...
    const newUser: IUser = {
      ...
      phone: otpDoc.phone ? Number(otpDoc.phone) : undefined,
      role: EnumUserRole.COMPANY,
      referralCode: referralCode,
      referredBy: referredBy?._id,
    };
    console.log(newUser, 'new user fro copmany');
