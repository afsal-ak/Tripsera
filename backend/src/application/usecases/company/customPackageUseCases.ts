...
export class CustomPackageUseCases implements ICustomPkgUseCases {
  ...
  async getAllRequestedCustomPkg(
    ...
    const result = await this._customPkgRepo.getAllRequestedCustomPkg(page, limit, filters,companyId);
    return {
  ...
  async changeCustomPkgStatus(
    ...
    const customPkg = await this._customPkgRepo.changeStatusAndResponse(customPkgId, data,companyId);
    return customPkg ? CustomPkgMapper.toResponseDTO(customPkg) : null;
  }
  async deleteCustomPkg(customPkgId: string,companyId:string,
): Promise<boolean> {
    return await this._customPkgRepo.delete(customPkgId);
  }

  async createCustomPackage(pkg: CompanyCreateCustomPackageDTO): Promise<PackageResponseDTO> {
    try {
      ...
      await this._notificationUseCases.sendNotification({
        ...
        message: 'Created new Custom Package',
