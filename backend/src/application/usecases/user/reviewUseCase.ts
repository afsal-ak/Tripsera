...
export class ReviewUseCases implements IReviewUseCases {
    ...
    async getUserReview(userId: string, page: number, limit: number): Promise<{
        data: IReview[]; pagination: PaginationInfo;
    }> {
        return await this.reviewRepo.findUserReviews(userId, page, limit)
    }

    async getPackageReviews(packageId: string, page: number, limit: number): Promise<{ review: IReview[]; pagination: PaginationInfo }> {
