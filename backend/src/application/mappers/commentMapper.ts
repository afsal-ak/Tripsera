...
export class CommentMapper {
  static toResponseDTO(comment: IComment): CommentResponseDTO {
    return {
      ...
      replyUser:
        typeof comment.userId === 'object' ? comment.userId.toString() : (comment.userId as string),
      text: comment.text,
   ...
   */
  static toResponseWithReplyCountDTO(comment: ICommentWithReplyCount): CommentResponseDTO {
    return {
      _id: comment._id?.toString() ?? '',
      parentId: comment.parentId?.toString() ?? '',
      parentType: comment.parentType,
