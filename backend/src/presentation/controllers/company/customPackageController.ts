...
import { ICustomPkgUseCases } from '@application/useCaseInterfaces/company/ICustomPackageUseCases';
import { CompanyCreateCustomPackageDTO, CompanyEditCustomPackageDTO, UpdateCustomPkgStatusDTO } from '@application/dtos/CustomPkgDTO';
...
export class CustomPackageController {
  ...
  changeCustomPkgStatus = async (
    ...
    try {
      ...
      const pkg = await this._customPkgUseCases.changeCustomPkgStatus(companyId,pkgId, data);
  ...
  getAllRequestedCustomPkgs = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const data = await this._customPkgUseCases.getAllRequestedCustomPkg(companyId, page, limit, filters);
  ...
  getAllApprovedCustomPkgs = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const data = await this._customPkgUseCases.getApprovedCustomPackage(companyId, page, limit, filters);
      res.status(HttpStatus.OK).json({
        data,
        message: CustomPackageMessages.PACKAGE_FETCHED,
      });
  ...
  deleteCustomPkg = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const pkgId = req.params.packageId;
      const result = await this._customPkgUseCases.deleteCustomPkg(pkgId, companyId);
      res.status(HttpStatus.OK).json({
        result,
  ...
  createCustomPackage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      let data: CompanyCreateCustomPackageDTO = req.body;
      let pkgData = {
        ...data,
        companyId
      }
  ...
  editCustomPackage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      let data: CompanyEditCustomPackageDTO = body;
     let pkgData = {
        ...data,
        companyId
      }
