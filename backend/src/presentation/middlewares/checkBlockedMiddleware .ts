...
export const checkBlockedMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (req.params.userId) {
      ...
      if (room) {
        targetUserId = room.participants.find((id) => id.toString() !== currentUserId)?.toString();
      }
    }
