...
const PackageCard = ({ pkg }: Props) => {
  ...
  const image =
    pkg.imageUrls?.[0]?.url.replace('/upload/', '/upload/f_auto,q_auto/') || '';

  // Check if offer is active and valid
  const now = new Date();
  const hasValidOffer =
    pkg.offer?.isActive && new Date(pkg.offer.validUntil) > now;

  // Calculate final price
  let finalPrice = pkg.finalPrice ?? pkg.finalPrice;
