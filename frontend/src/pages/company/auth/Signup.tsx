...
import { Link, useNavigate, useSearchParams } from 'react-router-dom';
 import { handleCompanyPreRegister } from '@/services/company/companyAuthService';
...
const CompanySignup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await handleCompanyPreRegister(email, username, password, refferalCode);
      localStorage.setItem('signupEmail', email);
      toast.success('OTP sent to your email');
      console.log(email, 'preregistration');

      navigate('/company/verify-otp');
    } catch (error: any) {
      ...
      <div className="flex w-full max-w-5xl shadow-lg rounded-lg overflow-hidden bg-white">
        ...
        <div className="w-full md:w-1/2 flex items-center justify-center p-8">
          <form onSubmit={handleSubmit} className="w-full max-w-md">
            <h2 className="text-2xl font-bold text-orange mb-6 text-center">Create an Company Account</h2>
            ...
            <p className="mt-4 text-sm text-center">
              ...
              <Link to="/company/login" className="text-orange underline">
                Login
              </Link>
            </p>
          </form>
