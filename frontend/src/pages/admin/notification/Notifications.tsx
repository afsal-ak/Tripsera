...
//   );
//   const adminId = useSelector((s: RootState) => s.adminAuth.admin?._id);
...
//   switch (n.entityType) {
//     case "booking":
//       return `/admin/bookings/${n.bookingId}`;
//     case "follow":
...
//     switch (n.entityType) {
//       case "booking":
//   return `/admin/bookings/${n.bookingId?._id || n.bookingId}`;
//       case "follow":
//         return `/admin/users/${n.triggeredBy}`;
//       case "wallet":
//         return `/admin/wallet`;
...
//             onClick={() => setFilter(f)}
//             className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors
//               ${filter === f ? "bg-blue-500 text-white shadow" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
//           >
//             {f === "all" ? "All" : f === "read" ? "Read" : "Unread"}
//           </button>
//         ))}
...
//             <div
//               key={n._id}
//               className={`flex items-center justify-between border rounded-xl p-4 shadow-sm transition hover:shadow-md ${
//                 n.isRead ? "bg-gray-50" : "bg-white"
//               }`}
//             >
...
//                   className="text-xs flex items-center gap-1 text-white bg-blue-500 hover:bg-blue-600 rounded-md px-3 py-1"
//                 >
  ...
  const getNotificationLink = (n: INotification) => {
    switch (n.entityType) {
      case "booking":
        return `/admin/bookings/${n.bookingId?._id || n.bookingId}`;
      case "package":
        return `/admin/packages/${n.packageId}`;
      case "review":
