...
import { fetchCouponData, changeCouponStatus } from '@/services/admin/couponService';
...
const CouponList = () => {
  ...
  const handleToggleBlock = async (id: string, isActive: boolean) => {
    try {
      ...
      toast.success(isActive ? 'Coupon activated successfully' : 'Coupon blocked successfully');
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <Button onClick={() => navigate('/admin/coupon/add')}>Add Coupons</Button>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            ...
            {coupons.map((coupon, index) => (
              <TableRow key={coupon._id}>
                ...
                <TableCell className="flex gap-2">
                  <ConfirmDialog
                    title={coupon.isActive ? 'Block this Coupon?' : 'Unblock this Coupon?'}
                    actionLabel={coupon.isActive ? 'Block' : 'Unblock'}
                    onConfirm={() => handleToggleBlock(coupon._id, !coupon.isActive)}
                  >
                    <Button
                      size="sm"
                      variant={coupon.isActive ? 'destructive' : 'outline'}
                      className={coupon.isActive ? '' : 'text-green-600 border-green-600'}
                    >
