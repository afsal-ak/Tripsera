...
const { data: homeData } = useQuery({
  ...
  const banners: IBanner[] = homeData?.result?.banners || [];

  //  TANSTACK - PACKAGES
 const { data: pkgData } = useQuery({
  queryKey: ['activePackages'],
  queryFn: () => fetchActivePackages({ limit: 4 }),
  staleTime: ONE_HOUR,
  gcTime: ONE_HOUR,
});

  const packages: IPackage[] = pkgData?.data || [];

  // TANSTACK - BLOGS
const { data: blogData } = useQuery({
  queryKey: ['blogs'],
  queryFn: () => fetchAllPublishedBlog(1, 4, ''),
  staleTime: ONE_HOUR,
  gcTime: ONE_HOUR,
});

  const blogs: IBlog[] = blogData?.data || [];
