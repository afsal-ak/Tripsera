...
export const fetchSalesReports = async (filters: ReportFilter) => {
  const res = await api.get('/company/salesReport', { params: filters });
  return res.data;
};

export const downloadSalesReportExcel = async (filters: ReportFilter) => {
  const res = await api.get('/company/salesReport/excel/download', {
    params: filters,
    responseType: 'blob',
  });
...
export const downloadSalesReportPDF = async (filters: ReportFilter) => {
  const res = await api.get('/company/salesReport/pdf/download', {
    params: filters,
