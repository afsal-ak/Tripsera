...
export const addPackage = async (formData: FormData) => {
  const response = await api.post('/company/addPackage', formData, {
    headers: {
...
export const fetchPackagesData = async (page: number, limit: number, filter: IFilter) => {
  ...
  const response = await api.get(`/company/packages`, { params });
...
export const blockPackage = async (id: string) => {
  const response = await api.patch(`/company/packages/${id}/block`);
  return response;
};
export const unBlockPackage = async (id: string) => {
  const response = await api.patch(`/company/packages/${id}/unblock`);
  return response;
};

export const getPackageById = async (id: string) => {
  const response = await api.get(`/company/packages/${id}`);
...
export const updatePackage = async (id: string, formData: FormData) => {
  try {
    const response = await api.put(`/company/packages/${id}/edit`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export const getCategory = async () => {
  ...
  console.log(response,'in frontend cat');
