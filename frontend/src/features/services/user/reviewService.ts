...
export const handleAddReview=async(packageId:string,rating:number,comment:string)=>{
    const response=await userApi.post(`/packages/${packageId}/reviews`,{rating,comment})
    return response.data
}
